...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    setIsLoading(true);
    try {
      // 1. Login popup Google Firebase di sisi Front-End
      const result = await signInWithPopup(auth, googleProvider);
      const user = result.user;

      // 2. Ambil Firebase ID Token dari user yang sukses login
      const idToken = await user.getIdToken();

      // 3. Kirim idToken ke backend Express kamu lewat instance API Axios
      ...
      // 4. Simpan session dari Backend ke LocalStorage
      ...
      // Memperbaiki urutan: alert dulu baru throw agar user tahu apa yang salah
